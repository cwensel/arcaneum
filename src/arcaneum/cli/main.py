...
@click.option('--verbose', '-v', is_flag=True, help='Verbose output')
@click.option('--json', 'output_json', is_flag=True, help='Output JSON format')
def index_pdfs(path, collection, model, workers, ocr_enabled, ocr_language, force, verbose, output_json):
    """Index PDF files to Qdrant collection (from RDR-004)"""
    from arcaneum.cli.index_pdfs import index_pdfs_command
    index_pdfs_command(path, collection, model, workers, ocr_enabled, ocr_language, force, verbose, output_json)
