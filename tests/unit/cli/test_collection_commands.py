...
class TestCollectionList:
    ...
    def test_empty_collections(self, mock_qdrant_client, mock_interaction_logger, capsys):
        ...
        with patch(
            "arcaneum.cli.collections.create_qdrant_client", return_value=mock_qdrant_client
        ):
    ...
    def test_with_collections(
        self, mock_qdrant_client_with_collections, mock_interaction_logger, capsys
    ):
        """Test listing when collections exist."""
        from arcaneum.cli.collections import list_collections_command

        with patch(
            "arcaneum.cli.collections.create_qdrant_client",
            return_value=mock_qdrant_client_with_collections,
        ):
            with patch(
                "arcaneum.cli.collections.get_collection_metadata",
                return_value={"model": "stella", "collection_type": "pdf"},
            ):
                list_collections_command(verbose=False, output_json=True)
        ...
        with patch(
            ...
            with patch("arcaneum.indexing.verify.CollectionVerifier") as mock_verifier_class:
                ...
                with patch(
                    ...
                    verify_collection_command(
                        "TestCollection", project=None, verbose=False, output_json=True
                    )

        # Parse the captured output as JSON
        output_str = output_buffer.getvalue()
        # Verify that the function was called with correct params
        mock_verifier.verify_collection.assert_called_once_with(
            "TestCollection", project_filter=None, verbose=False
        )
    ...
    def test_project_filter(self, mock_qdrant_client, mock_interaction_logger):
        ...
        with patch(
            ...
            with patch("arcaneum.indexing.verify.CollectionVerifier") as mock_verifier_class:
                ...
                verify_collection_command(
                    "CodeCollection", project="myrepo#main", verbose=False, output_json=True
                )

        # Verify the filter was passed
        mock_verifier.verify_collection.assert_called_once_with(
            "CodeCollection", project_filter="myrepo#main", verbose=False
        )
...
class TestCollectionCreate:
    ...
    def test_create_with_type(self, mock_qdrant_client, mock_interaction_logger):
        ...
        mock_qdrant_client.get_collection.return_value = MagicMock(
            config=MagicMock(params=MagicMock(vectors={"arctic-m": MagicMock(size=768)}))
        )
        ...
        with patch(
            ...
            with patch("arcaneum.cli.collections.build_vectors_config", return_value={}):
                with patch("arcaneum.cli.collections.set_collection_metadata"):
                    with patch(
                        "arcaneum.cli.collections.EMBEDDING_MODELS",
                        {"arctic-m": {"dimensions": 768}},
                    ):
