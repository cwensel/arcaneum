...
def test_corpus_repair_policy_migration_is_opt_in():
    ...
    def fake_sync_directory_command(*args, **kwargs):
        calls.append(kwargs["repair_stale_policy"])

    patches = {
        "arcaneum.cli.sync.sync_directory_command": fake_sync_directory_command,
        "arcaneum.cli.utils.create_qdrant_client": lambda: object(),
        "arcaneum.indexing.collection_metadata.get_collection_metadata": (
            lambda _client, _corpus: {"model": "arctic-m"}
        ),
    }

    default_result = _run(["corpus", "repair", "Docs"], **patches)
    migration_result = _run(["corpus", "repair", "Docs", "--include-stale-policy"], **patches)
    ...
    with patch("arcaneum.cli.index_source.interaction_logger"):
        with patch("arcaneum.cli.index_source.create_qdrant_client", return_value=object()):
            with patch("arcaneum.cli.index_source.QdrantIndexer", return_value=mock_indexer):
                with patch(
                    "arcaneum.cli.index_source.get_vector_names", return_value=["jina-code"]
                ):
                    with patch(
    ...
    with patch("arcaneum.cli.index_source.interaction_logger"):
        with patch("arcaneum.cli.index_source.create_qdrant_client", return_value=object()):
            with patch("arcaneum.cli.index_source.QdrantIndexer", return_value=mock_qdrant_indexer):
                with patch(
                    "arcaneum.cli.index_source.SourceCodeIndexer", return_value=mock_source_indexer
                ):
                    with patch(
                        "arcaneum.cli.index_source.get_vector_names", return_value=["jina-code"]
                    ):
